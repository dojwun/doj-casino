...
local function tryTakeChips(source,amount)
    local canBet = false
    local item = exports.ox_inventory:GetItem(source, 'casinochips', nil, true)
    if item then
        if exports.ox_inventory:RemoveItem(source, 'casinochips', amount) then
            exports.ox_inventory:RemoveItem(source, 'casinochips', amount)
            canBet = true
        else
            canBet = false
        end
...
AddEventHandler("Blackjack:requestSitAtBlackjackTable", function(chairId)
    ...
    if source ~= nil then
        ...
        TriggerClientEvent('ox_lib:notify', source, {type = 'error', description = "cant sit you down."})
    end
end)
...
AddEventHandler("Blackjack:setBlackjackBet",function(gameId,betAmount,chairId)
    ...
    if gameId ~= nil and betAmount ~= nil and chairId ~= nil then 
            ...
            if tonumber(betAmount) then
                ...
                if betAmount > 0 then
                    if tryTakeChips(source, betAmount) then
                        ...
                        TriggerClientEvent('ox_lib:notify', source, {type = 'success', description = 'Bet placed: ' .. tostring(betAmount) .. ' chips.'})

                    else  
                        TriggerClientEvent('ox_lib:notify', source, {type = 'error', description = 'You dont have enough chips.'})
        ...
        TriggerClientEvent('ox_lib:notify', source, {type = 'error', description = 'Error betting!'})
    end
end)
                                        ...
                                        while nextCardCount >= 1 do
                                            ...
                                            if blackjackGameData[gameId][source][2][nextCardCount] == true then --if hit 
                                                ...
                                                if currentHand > 21 then
                                                    ...
                                                    TriggerClientEvent('ox_lib:notify', source, {type = 'error', description = 'Lost -'..tostring(lostAmount).." chips"})
                                            ...
                                            if currentHand <= 21 then
                                                ...
                                                if dealerHand > 21 then
                                                    ...
                                                    if playerPing ~= nil then
                                                        if playerPing > 0 then
                                                            ...
                                                            TriggerClientEvent('ox_lib:notify', source, {type = 'success', description = 'Dealer Bust! BLACKJACK! +'..tostring(potentialWinAmount)..' chips', 'success'})
                                                            -- PlaySoundFrontend(source, "CHALLENGE_UNLOCKED", "HUD_AWARDS", 1) 
                                                ...
                                                elseif currentHand > dealerHand and currentHand <= 21 then
                                                    ...
                                                    if playerPing ~= nil then
                                                        if playerPing > 0 then
                                                            TriggerClientEvent("Blackjack:blackjackWin",source,tableId)
                                                            TriggerClientEvent('ox_lib:notify', source, {type = 'success', description ='BLACKJACK! +'..tostring(potentialWinAmount)..' chips', 'info'})
                                                            -- PlaySoundFrontend(source, "TENNIS_MATCH_POINT", "HUD_AWARDS", 1)
                                                ...
                                                elseif currentHand == dealerHand then
                                                    ...
                                                    if playerPing ~= nil then
                                                        if playerPing > 0 then
                                                            ...
                                                            TriggerClientEvent('ox_lib:notify', source, {type = 'success', description ='You pushed!', 'info'})
                                                            -- PlaySoundFrontend(source, "ERROR", "HUD_AMMO_SHOP_SOUNDSET", 1)
                                                        end
                                                    end
                                                else
                                                    ...
                                                    if playerPing ~= nil then
                                                        if playerPing > 0 then
                                                            ...
                                                            TriggerClientEvent('ox_lib:notify', source, {type = 'success', description ='You lost! -'..tostring(potentialPushAmount)..' chips', 'warning'})
                                                            -- PlaySoundFrontend(source, "ERROR", "HUD_AMMO_SHOP_SOUNDSET", 1)
